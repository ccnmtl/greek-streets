...
def get_photos():
    ...
    photo_pages = {}
    prefix = PHOTO_DIR + '/34_FILES_360_ATHENS_EXARCHIA'

    for obj in my_bucket.objects.filter(Prefix=prefix):
        # Only look at files in the PHOTO directories, not the html
        # files.
        if ('/PHOTO/' not in obj.key) or obj.key.endswith('.html'):
            continue
        ...
        if not photo_pages.get(pagename):
            photo_pages[pagename] = [public_url]
        else:
            photo_pages[pagename].append(public_url)
...
def main():
    photo_pages = get_photos()
    print(photo_pages.keys())

    make_photo_pages(photo_pages)
